...
const span = document.getElementById('splashSpan');
const root = document.querySelector(':root');
root.style.setProperty('--text-width', '2vw');
span.innerText = splashes[randNum(0, splashes.length-1)];

function spamCharacters() {
    let text = "";
    text += pixel5[randNum(0, pixel5.length-1)]
    text += pixel5[randNum(0, pixel5.length-1)]
    text += pixel5[randNum(0, pixel5.length-1)]
    text += pixel5[randNum(0, pixel5.length-1)]
    text += pixel5[randNum(0, pixel5.length-1)] + " "
...
function writeHTMLForCharacter(color, char) {
    return `<span style="text-shadow:calc(var(--text-width)/10) calc(var(--text-width)/10) ${colorShadows[colorList.indexOf(color)]};color:${colors[colorList.indexOf(color)]}">${char}</span>`
}

function colorCharacters() {
    let text = span.innerText
    let newText = ""
    let i = 1
    newText += writeHTMLForCharacter(text[i], text[i+1]);
    i += 3;
    newText += writeHTMLForCharacter(text[i], text[i+1]);
    i += 3;
    newText += writeHTMLForCharacter(text[i], text[i+1]);
...
if (span.innerText.includes('§k')){
    span.innerText = span.innerText.replaceAll("§k", "")
    spamCharacters()
} else if (span.innerText.includes('§1')){
    colorCharacters()
}

function setFontSize() {
    let width = 1100 / span.offsetWidth > 5 ? 5 : 1000 / span.offsetWidth;
    root.style.setProperty('--text-width', width + 'vw');
    span.style.opacity = 1
    span.style.animationPlayState = "unset"
}
...
document.querySelectorAll("#titleScreen button").forEach((element)=>{
    element.addEventListener("mouseover", (event) => {
        const clone = hover.cloneNode(true);
        clone.play();
    });

    element.addEventListener("mousedown", (event) => {
        const clone = click.cloneNode(true);
        clone.play();
        document.getElementById('titleScreen').style.display = "none"
        document.getElementById('menuScreen').style.display = "inherit"
        loadMenu(element.innerText)
    });
})

document.getElementById('back').addEventListener("mousedown", (event) => {
    const clone = back.cloneNode(true);
    clone.play();
    document.getElementById('titleScreen').style.display = "inherit"
    document.getElementById('menuScreen').style.display = "none"
});

socials = [["Youtube", "https://www.youtube.com/jerrinth3glitch"], ["Speedrun.com", "https://www.speedrun.com/users/jerrinth3glitch"], ["Discord", "https://discord.gg/vGW4pSF8wc"], ["Replit", "https://replit.com/@JerrinShirks"], ["Github", "https://github.com/zugebot"]]
games = [["The Rock Dating Sim", "https://www.jerrin.org/apps/rockdatingsim.html"], ["Chuzzles", "https://www.jerrin.org/apps/chuzzles.html"], ["Isacar's Dream", "https://www.jerrin.org/apps/app3.html"]]
software = [["Jerrin's Retiming Tool", "https://www.jerrin.org/downloads/retimer/download_page.html"]]
...
document.addEventListener("keydown", (event) => {
    if (event.code == "Escape" && document.getElementById('menuScreen').style.display == "inherit") {
        const clone = back.cloneNode(true);
        clone.play();
        document.getElementById('titleScreen').style.display = "inherit"
        document.getElementById('menuScreen').style.display = "none"
