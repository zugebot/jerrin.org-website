// forked from Sprinkz, edited by Jerrin Shirks, refactored by Fir
...
(function () {
    ...
    if (! name.includes("bilibili.") && 
        ! name.endsWith("twitch.tv") && 
        ! name.endsWith("youtube.com") && 
    ...
    function o(t, media) {
        let media_time_diff = Math.abs(media.mediaTime - last_media_time);
        let frame_num_diff = Math.abs(media.presentedFrames - last_frame_num);
    ...
    addEventListener("keydown", async function (e) {
        if (',.'.includes(event.key)) {
            if (!location.hostname.endsWith("youtube.com")) {
                vid.currentTime += (event.key === ',' ? -1 : 1) / fps
                vid.playbackRate = 1
                return false
            }
        }
        ...
        if (event.key === "t") {
            let time = m(last_media_time).toString()
            this.navigator.clipboard.writeText(time)
            .then(() => {
                alert('Copied! ' + time);
            })
        }
